...
const ResourceCard = ({ title, desc, icon, link, tags }: any) => (
  <a href={link} className="group block bg-card/40 backdrop-blur-sm border border-white/10 rounded-xl p-6 hover:border-primary/50 hover:shadow-lg hover:shadow-primary/5 transition-all h-full">
    <div className="flex items-start justify-between mb-4">
      <div className="p-3 rounded-lg bg-white/5 text-white group-hover:bg-primary group-hover:text-black transition-colors">
        {icon}
      </div>
    ...
    <div className="flex flex-wrap gap-2">
      {tags.map((tag: string, i: number) => (
        <span key={i} className="text-[10px] uppercase font-mono px-2 py-1 rounded bg-white/5 text-slate-400 border border-white/5">
          {tag}
        </span>
      ))}
    </div>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-20">
        <div className="mb-16">
          <h1 className="text-4xl font-display font-bold text-white mb-6">Learning Hub</h1>
        ...
        <section className="mb-20">
          <div className="flex items-center gap-3 mb-8 border-b border-white/10 pb-4">
            ...
            <h2 className="text-2xl font-bold text-white">Workshop Materials</h2>
          </div>
          <div className="bg-card/20 border border-white/10 rounded-xl overflow-hidden">
            {[
              ...
              <div key={i} className="flex items-center justify-between p-4 border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors group">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-slate-500">{item.date} • PDF</span>
                  </div>
                </div>
        ...
        <section>
          <div className="flex items-center gap-3 mb-8 border-b border-white/10 pb-4">
            ...
            <h2 className="text-2xl font-bold text-white">Featured Talks</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[1, 2, 3].map((_, i) => (
              <div key={i} className="group relative aspect-video bg-black rounded-xl overflow-hidden border border-white/10 hover:border-primary/50 transition-all cursor-pointer">
